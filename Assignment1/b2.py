...
def plot_timedomain(wavfile):
    '''
        画出时域图
    '''
    wave_params, wave_data = fileRead(wavfile)  # 获取处理好的wave数据
    framerate = wave_params[2]  # 获取帧率
    nframes = wave_params[3]  # 获取帧数
...
def plot_freqdomain(start, fft_size, wavfile):
    ...
    fft_y1 = np.fft.fft(wave_data[0][start:start+fft_size-1])/fft_size  # 左声部
    fft_y2 = np.fft.fft(wave_data[1][start:start+fft_size-1])/fft_size  # 右声部
    # print('fft_y1',fft_y1[:5])
    # print('fft_y2',fft_y2[:5])
    # 2.计算频域图x值
